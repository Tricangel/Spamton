...
public class LetterEditScreen extends Screen {
    ...
    private static final Component SIGN_LETTER_LABEL = Component.translatable("book.signButton");
    ...
    public LetterEditScreen(Player player, ItemStack itemStack, InteractionHand interactionHand, WritableBookContent writableBookContent) {
        super(TITLE);
        this.numberOfPages = CommonComponents.EMPTY;
        this.owner = player;
        this.book = itemStack;
        this.hand = interactionHand;
        Stream<String> bookPages = writableBookContent.getPages(Minecraft.getInstance().isTextFilteringEnabled());
        List<String> pages1 = this.pages;
        Objects.requireNonNull(pages1);
        bookPages.forEach(pages1::add);
        if (this.pages.isEmpty()) {
            this.pages.add("");
        }

        this.signScreen = new LetterSignScreen(this, player, interactionHand, pages);
    }
    ...
    protected void init() {
        ...
        this.addRenderableWidget(Button.builder(SIGN_LETTER_LABEL, (button) -> this.minecraft.setScreen(this.signScreen)).pos(this.width / 2 - 98 - 2, this.menuControlsTop()).width(98).build());
    ...
    private void updateButtonVisibility() {
        ...
        this.forwardButton.visible = this.currentPage < 2;
    }
    ...
    public boolean keyPressed(KeyEvent keyEvent) {
        return switch (keyEvent.key()) {
            ...
            default -> super.keyPressed(keyEvent);
        };
    }
